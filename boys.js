...
async function pageSetup() {
	...
	for (let i = 0; i < num; i++) {
		...
		counter.innerHTML = '<span id="' + i + '-h">xx</span>h <span id="' + i + '-m">xx</span>m <span id="' + i + '-s">xx</span>s';
...
function checkerRotTod(input, elementId) {
	if (input.hours < ROT.h || (input.hours <= ROT.h && input.minutes < ROT.m) || (input.hours <= ROT.h && input.minutes <= ROT.m && input.seconds < ROT.s)) {
		document.getElementById(elementId).setAttribute("class", "boys timer rot");
	}
	if (input.hours == 0 && input.minutes == 0 && input.seconds == 0) {
		document.getElementById(elementId).setAttribute("class", "boys timer rot tot");
	}
}
